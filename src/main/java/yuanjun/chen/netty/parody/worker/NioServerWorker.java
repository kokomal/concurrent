...
public class NioServerWorker extends AbstractNioSelector implements Worker {
    ...
    protected void process(Selector selector) throws IOException {
        ...
        while (ite.hasNext()) {
            ...
            ite.remove();
            // 得到事件发生的Socket通道
            SocketChannel channel = (SocketChannel) key.channel(); // 为何这里直接知道这是READ?
            // 数据总长度
            int ret = 0;
    ...
    public void registerNewChannelTask(final SocketChannel channel) {
        ...
        registerTask(new Runnable() {  // taskQueue里面存放的是通道注册任务
            @Override
            public void run() {
                try {
                    // 将客户端注册到selector中
                    channel.register(selector, SelectionKey.OP_READ); // 关键，Worker永远关心READ事件
